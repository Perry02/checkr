...
```bash
PHASE_INFO=$(node "D:/checkr/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "D:/checkr/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "D:/checkr/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
