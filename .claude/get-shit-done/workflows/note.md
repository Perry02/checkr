...
- **Global scope**: `D:/checkr/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/checkr/.claude/notes/`)
...
2. Glob `D:/checkr/.claude/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `D:/checkr/.claude/notes/` — works in any directory
