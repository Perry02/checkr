...
use ce_core::gn::compiler_gen::{CompilerContext, gen_commands, gen_commands_for_level, generate_witness_memories};
...
/// Generate a compiler input restricted to scenarios at or below `level`.
/// Level 1=Assignment, 2=Sequencing, 3=Conditionals, 4=Loops,
...
/// Is there an if-fi directly inside a do-od body (NestedIfInDo)?
...
/// Is there a do-od directly inside an if-fi body (NestedDoInIf)?
...
fn diagnose_mismatch(
    input: &Input,
    commands: &Commands,
    o_g: &dot::ParsedGraph, // reference (oracle)
    t_g: &dot::ParsedGraph, // student output
) -> String {
    ...
    // Array read (x := A[i]) — shows up as an assignment count/fingerprint mismatch
    ...
    // Edge counts match but fingerprints/paths differ — semantic error in guard/assignment expressions
